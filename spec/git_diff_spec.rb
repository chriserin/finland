...
+  booking.parking_period = (20.days.from_now..(20.days.from_now + 3.hours))
    ...
    expect(result).to eq({Dir.pwd + "/" + "db/seeds/development.rb" => [155..156]})
  end
...
+  booking.parking_period = (4.days.from_now..(4.days.from_now + 3.hours))
    ...
    result = Finland::GitDiff.parse(git_diff)
    expect(result).to eq({Dir.pwd + "/" + "db/seeds/development.rb" => [155..156, 174..175, 192..193, 210..211]})
  end
...
+      post_time: Time.zone.now,
    ...
    result = Finland::GitDiff.parse(git_diff)
    expect(result).to eq({Dir.pwd + "/" + "app/models/booking.rb" => [108..108, 112..112], Dir.pwd + "/" + "app/models/cancellation.rb" => [34..35, 43..44]})
  end
end
